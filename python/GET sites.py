...
def get_all_sites(api_version: str, token: str):
    """Get all sites if user has Server Admin access."""
    url = f"{SERVER_URL}/api/{api_version}/sites?pageSize=1000&pageNumber=1"
    headers = {'X-Tableau-Auth': token}
    response = requests.get(url, headers=headers)
    response.raise_for_status()
    root = ET.fromstring(response.text)
    ns = {'t': 'http://tableau.com/api'}
    return root.findall('.//t:site', ns)

def get_site_details(api_version: str, token: str, site_id: str):
    """Get details for a specific site."""
    url = f"{SERVER_URL}/api/{api_version}/sites/{site_id}"
    headers = {'X-Tableau-Auth': token}
    response = requests.get(url, headers=headers)
    response.raise_for_status()
    root = ET.fromstring(response.text)
    ns = {'t': 'http://tableau.com/api'}
    return root.find('.//t:site', ns)

def parse_site(site_elem):
    """Parse site XML element into flat dictionary."""
    site_info = {}
    # Top-level attributes
    for key, value in site_elem.attrib.items():
        site_info[key] = value

    # Flatten known nested children into explicit columns
    for child in site_elem:
        tag = child.tag.split('}')[-1]  # strip namespace
        for k, v in child.attrib.items():
            site_info[f"{tag}_{k}"] = v

    return site_info

def fetch_rows(api_version: str, site_id: str, token: str):
    """Fetch site information - try all sites first, fallback to current site."""
    site_elements = []
    
    # Try multi-site, else fallback to single site
    try:
        site_elements = get_all_sites(api_version, token)
        print_progress(f"Found {len(site_elements)} sites (Server Admin access)")
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 403:
            print_progress("403 Forbidden → Falling back to just the signed-in site")
            single_site_elem = get_site_details(api_version, token, site_id)
            site_elements = [single_site_elem]
        else:
            raise

    # Parse all sites
    rows = [parse_site(site_elem) for site_elem in site_elements]
    print_progress(f"Total sites: {len(rows)}")
    return rows
...
# ==============================
def main():
    with ScriptTimer("REGEN_GET_items_sites"):
